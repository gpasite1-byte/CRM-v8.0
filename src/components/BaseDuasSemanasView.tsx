...
import { Search, Plus, Filter, Database, FileSpreadsheet, TrendingUp, CheckCircle, XCircle, Clock, Sparkles, Edit3, Trash2, Save, X, Lock, ShieldCheck } from 'lucide-react';
...
export default function BaseDuasSemanasView({
  ...
  // Base state initialized from localStorage or baseDuasSemanasData
  ...
  useEffect(() => {
    const handleSync = () => {
      try {
        const saved = localStorage.getItem('gpa_base_duas_semanas');
        if (saved) {
          setLocalPropostas(JSON.parse(saved));
        }
      } catch (err) {
        console.error(err);
      }
  ...
  // Combine static proposals with dynamic deals from CRM Pipeline
  const combinedPropostas = useMemo(() => {
    const list: BasePropostaRow[] = localPropostas.map(p => ({
      ...p,
    ...
    deals.forEach(d => {
      const exists = list.some(p => 
        p.cliente.toLowerCase() === (d.clienteNome || d.titulo).toLowerCase() && 
        p.gestorComercial.toLowerCase() === d.comercialNome.toLowerCase()
      );
      ...
      if (!exists) {
        ...
        list.push({
          ...
          gestorComercial: d.comercialNome || 'Comercial',
  ...
  const gestoresUnicos = useMemo(() => {
    const list = Array.from(new Set(combinedPropostas.map(p => p.gestorComercial)));
    return ['Todos', ...list];
  }, [combinedPropostas]);

  const estadosUnicos = useMemo(() => {
    const list = Array.from(new Set(combinedPropostas.map(p => p.estadoProposta)));
    return ['Todos', ...list];
  }, [combinedPropostas]);
  ...
  return (
    <div className="w-full space-y-4 font-sans text-slate-900 dark:text-slate-100 my-2">
      ...
      <div className={`p-2.5 rounded-sm text-xs font-sans font-semibold flex items-center gap-2 border ${
        isAdmin 
          ? 'bg-amber-50 text-amber-900 border-amber-200' 
          : 'bg-blue-50 text-blue-900 border-blue-200'
      }`}>
        <ShieldCheck size={16} className={isAdmin ? 'text-amber-600 shrink-0' : 'text-blue-600 shrink-0'} />
        <span>
          {isAdmin ? (
            <>👑 <strong>Modo Administrador ({loggedUser?.nome || 'Admin'}):</strong> Tem acesso total para criar, editar e eliminar qualquer registo na Base de Dados.</>
          ) : (
            <>🔒 <strong>Permissão Comercial ({loggedUser?.nome || 'Comercial'}):</strong> Pode adicionar novas propostas e gerir (editar/eliminar) <u>apenas os seus próprios dados</u>. Registos de outros comerciais são apenas para consulta.</>
          )}
        </span>
      </div>
              ...
              filteredPropostas.map((row, idx) => {
                ...
                return (
                  <tr key={`${semDisplay}-${row.id}-${row.cliente}-${idx}`} className="hover:bg-blue-50/60 dark:hover:bg-slate-800/80 transition-colors">
                    ...
                    <td className="px-2 py-1.5 text-center font-mono font-bold text-gray-800 border-r border-gray-300">
                      {row.id}
                    </td>
                    <td className="px-2.5 py-1.5 text-center text-gray-800 border-r border-gray-300 whitespace-nowrap font-mono text-[10px]">
                      {row.dataEnvio}
                    </td>
                    <td className="px-3 py-1.5 font-bold text-gray-900 border-r border-gray-300 whitespace-nowrap">
                      {row.cliente}
                    </td>
                    <td className="px-3 py-1.5 text-gray-800 border-r border-gray-300 font-medium">
                      {row.servico}
                    </td>
                    <td className="px-2.5 py-1.5 border-r border-gray-300 whitespace-nowrap">
                      <span className={`font-semibold ${
                        row.estadoProposta === 'Proposta aprovada' ? 'text-emerald-700' :
                        row.estadoProposta === 'Perdida' ? 'text-rose-700' :
                        row.estadoProposta === 'Proposta em negociação' ? 'text-amber-700' : 'text-blue-800'
                      }`}>
                        {row.estadoProposta}
                      </span>
                    </td>
                    <td className="px-3 py-1.5 text-right font-semibold text-gray-900 border-r border-gray-300 whitespace-nowrap font-mono">
                      {row.valorProposta}
                    </td>
                    <td className="px-3 py-1.5 text-right font-semibold text-emerald-800 border-r border-gray-300 whitespace-nowrap font-mono bg-emerald-50/20">
                      {row.valorAprovado}
                    </td>
                    <td className="px-3 py-1.5 text-right font-semibold text-rose-800 border-r border-gray-300 whitespace-nowrap font-mono bg-rose-50/20">
                      {row.valorPerdido}
                    </td>
                    <td className="px-2 py-1.5 text-center text-gray-800 border-r border-gray-300 font-mono font-bold">
                      {row.probabilidade}
                    </td>
                    <td className="px-2.5 py-1.5 text-right text-gray-800 border-r border-gray-300 whitespace-nowrap font-mono bg-amber-50/20 font-semibold">
                      {row.valorPonderado}
                    </td>
                    <td className="px-2.5 py-1.5 text-gray-800 border-r border-gray-300 whitespace-nowrap font-medium">
                      {row.gestorComercial}
                    </td>
                    <td className="px-3 py-1.5 text-gray-700 border-r border-gray-300 text-[10px]">
                      {row.proximaAccao}
                    </td>
                    <td className="px-2.5 py-1.5 text-center text-gray-800 border-r border-gray-300 whitespace-nowrap font-mono text-[10px]">
                      {row.proximoContacto}
                    ...
                    <td className="px-2 py-1.5 text-center font-mono text-gray-700 border-r border-gray-300">
                      {row.diasEmAberto}
                    </td>
                    <td className="px-2 py-1.5 text-center font-bold text-gray-800 border-r border-gray-300">
                      {row.classeCliente}
                    </td>
                    <td className="px-2.5 py-1.5 text-center border-r border-gray-300 whitespace-nowrap">
                      <span className={`font-bold ${
                        row.prioridade === 'Alta' ? 'text-rose-600' :
                        row.prioridade === 'Média' ? 'text-amber-600' : 'text-gray-700'
                      }`}>
                        {row.prioridade}
                      </span>
                    </td>
                    <td className="px-2.5 py-1.5 text-center font-medium border-r border-gray-300 whitespace-nowrap">
                      <span className={`px-1.5 py-0.5 rounded-xs text-[10px] font-bold ${
                        row.estadoCRM === 'Fechado ganho' ? 'bg-emerald-100 text-emerald-800 border border-emerald-300' :
                        row.estadoCRM === 'Fechado perdido' ? 'bg-rose-100 text-rose-800 border border-rose-300' :
                        'bg-amber-100 text-amber-800 border border-amber-300'
                      }`}>
                        {row.estadoCRM}
                      </span>
                    </td>
                    <td className="px-2.5 py-1.5 text-right text-gray-700 border-r border-gray-300 font-mono text-[10px]">
                      {row.metaSemanal}
                    </td>
                    <td className="px-2 py-1.5 text-center font-bold text-gray-800 font-mono text-[10px] border-r border-gray-300">
                      {row.pctMeta}
                    </td>
                    <td className="px-2 py-1.5 text-center">
                      <div className="flex items-center justify-center gap-1">
                        {canModifyRow(row) ? (
                          ...
                          <span 
                            className="p-1 text-gray-300 cursor-not-allowed flex items-center justify-center"
      ...
      {editingRow && (
        <div className="fixed inset-0 bg-black/60 z-[1000] flex items-center justify-center p-4 font-sans">
          <div className="bg-white rounded-md shadow-2xl max-w-lg w-full overflow-hidden border border-gray-300 animate-in fade-in zoom-in-95 duration-150">
            ...
            <form onSubmit={handleUpdateProposta} className="p-4 space-y-3 text-xs">
              <div className="grid grid-cols-2 gap-3">
                <div>
                  <label className="block font-bold text-gray-700 mb-1">Semana *</label>
                  <select 
                    value={editingRow.semana}
                    onChange={e => setEditingRow({ ...editingRow, semana: e.target.value })}
                    className="w-full border border-gray-300 rounded-sm p-2 bg-gray-50 focus:outline-hidden focus:border-[#1B365D]"
                  >
                ...
                <div>
                  <label className="block font-bold text-gray-700 mb-1">Comercial Responsável *</label>
                  <input 
                    type="text"
                    required
                    value={editingRow.gestorComercial}
                    onChange={e => setEditingRow({ ...editingRow, gestorComercial: e.target.value })}
                    className="w-full border border-gray-300 rounded-sm p-2 focus:outline-hidden focus:border-[#1B365D]"
                  />
                </div>
              </div>
              ...
              <div>
                <label className="block font-bold text-gray-700 mb-1">Serviço / Descrição *</label>
                <input 
                  type="text"
                  required
              ...
              <div className="grid grid-cols-2 gap-3">
                <div>
                  <label className="block font-bold text-gray-700 mb-1">Valor Proposto (AOA) *</label>
                  <input 
                    type="text"
                    required
                ...
                <div>
                  <label className="block font-bold text-gray-700 mb-1">Estado da Proposta</label>
                  <select 
              ...
              <div className="grid grid-cols-2 gap-3">
                <div>
                  <label className="block font-bold text-gray-700 mb-1">Nível de Prioridade</label>
                  <select 
                ...
                <div>
                  <label className="block font-bold text-gray-700 mb-1">Próxima Acção</label>
                  <input 
                    type="text"
              ...
              <div className="pt-2 flex items-center justify-end gap-2 border-t border-gray-200">
                <button 
                  type="button"
                  onClick={() => setEditingRow(null)}
                  className="px-3.5 py-1.5 border border-gray-300 rounded-sm text-gray-700 hover:bg-gray-100 font-bold cursor-pointer"
                >
                  Cancelar
                </button>
                <button 
      ...
      {isAddModalOpen && (
        <div className="fixed inset-0 bg-black/60 z-[1000] flex items-center justify-center p-4 font-sans">
          <div className="bg-white rounded-md shadow-2xl max-w-lg w-full overflow-hidden border border-gray-300 animate-in fade-in zoom-in-95 duration-150">
            ...
            <form onSubmit={handleCreateProposta} className="p-4 space-y-3 text-xs">
              <div className="grid grid-cols-2 gap-3">
                <div>
                  <label className="block font-bold text-gray-700 mb-1">Semana de Registo *</label>
                  <select 
                    value={newSemana}
                ...
                <div>
                  ...
                  <select 
                    ...
                    className="w-full border border-gray-300 rounded-sm p-2 bg-gray-50 focus:outline-hidden focus:border-[#1B365D] disabled:opacity-80 disabled:cursor-not-allowed font-medium"
                  >
